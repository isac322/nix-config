...
Nix에서 오는 GUI 예외는 Firefox와 서버 맥의 Camoufox·DeskPad·macVNC다.
Firefox는 오버레이가 주는 `.app` 번들이라
[정책을 두 경로로](0021-firefox-policies-two-paths.md) 넣어야 한다. Camoufox와
DeskPad는 LaunchAgent가 store에서 직접 실행한다. macVNC payload도 Nix package를
따르지만, 고정 출력 `Camofox VNC Host.app`이 별도 LaunchAgent에서 자식으로
실행한다. 따라서 Screen Recording과 Accessibility 권한은 바뀌는 Nix bash나
macVNC store path가 아니라 host의 고정 identity에 남는다
