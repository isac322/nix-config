...
머지되고 1.4.196에서 회귀가 해결됨을 실측 프로브로 확인하여 고정을 해제했다.
...
같은 래퍼가 Homebrew의 `orca` 실행 파일이 아직 없으면 0이 아닌 값으로 빠진다.
nix-darwin의 Homebrew activation과 Home Manager activation 사이에는 순서 보장이
없으므로, cask 설치가 끝난 뒤 `KeepAlive`가 다시 시도하게 한다.
