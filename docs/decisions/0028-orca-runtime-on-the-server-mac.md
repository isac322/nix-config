...
랩탑은 사람이 데스크톱 앱을 쓰므로 Homebrew cask를 계속 따른다. 서버에서는
클라이언트가 UI일 뿐이고 프로젝트·워크트리·터미널·에이전트 프로세스를 쥐는
런타임이 항상 살아 있어야 하므로 자동 업데이트보다 검증한 `serve`가 우선이다.
...
같은 래퍼가 Nix로 고정한 `orca` 실행 파일이 없을 때도 0으로 빠진다. 정상
generation에서는 있을 수 없지만, 실행 경로가 사라졌을 때 재시작 루프를 만들지는 않는다.
